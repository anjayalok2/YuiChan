...
const {Message, StringSession, Image, Video} = require('./mainbox/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('Activating  ' + config.WORKTYPE + 'work 👻'));
            await conn.sendMessage(conn.user.jid, "Bot all set", MessageType.text);
            await conn.sendMessage(conn.user.jid, "``` BOT IS NOW " + config.WORKTYPE + "```" , MessageType.text);
    });
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '__YuiChan_☠☠_[error spotted] ' +
                                    '\n\n*🕵️ ' + error + '*\n'
                                    , MessageType.text);
                            }
                        }
