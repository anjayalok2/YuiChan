...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Yui voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Nexus.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Yui') && conf.CHAT_BOT !== 'true') {        
        ...
        var finm = message.message.replace('Yui', '').replace(' ', '')   
...
Nexus.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.CHAT_BOT == 'true' && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
Nexus.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Yui özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Yui yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Yui yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Yui, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Yui, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Yui features. Turn your account into a ai chatbot!'
    already_on = 'Yui artificial intelligence is already fully functional.'
    already_off = 'Yui artificial intelligence is currently running semi-functional.'
    succ_on = 'Yui Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Yui Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Yui സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Yui കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Yui AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Yui പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Yui സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Nexus.addCommand({ pattern: 'yui ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.Yui on / off' }, (async (message, match) => {
    var Yui_status = `${conf.CHAT_BOT}`
    if (match[1] == 'on') {
        if (Yui_status == 'true') {
