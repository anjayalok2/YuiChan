...
if (Config.WORKTYPE == 'private') {
    ...
    Nexus.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
    ...
    Nexus.addCommand({pattern: 'meme ?(.*)', fromMe: false, desc: Lang.MEMES_DESC}, (async (message, match) => {    
    ...
    //created by afnanplk
	    ...
	    memeMaker({
            ...
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('MyPnky-meme.png'), MessageType.image, {filename: 'Nexusmeme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
    }		    
    }));
