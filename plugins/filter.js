...
Nexus.addCommand({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
...
Nexus.addCommand({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Nexus.addCommand({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
...
Nexus.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        ...
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '918129624395@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/media/bhoom.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted : message.data, ptt: true})
        }
...
const array = [ 'pro','pwoli','scene','sed','vaa','vanno','vannu','hi']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
...
const array = ['ayin','back','Back','Bot','Hehe','Hello','Hlo','Kill','kill','kiss','line','love','mwolu','Mwolu','single','tha','thund','z','Z','bie','Bie','Nexus','Riaz sempai','Yuichan','Aflah','nwakhil']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
...
async function checkImAdmin(message, user = message.client.user.jid) {
    ...
    return sonuc.includes(true);
     ...
     Nexus.addCommand({on: 'text', fromMe: false,onlyGroup: true}, (async (message, match) => {
